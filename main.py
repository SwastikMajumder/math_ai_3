    ...
    def does_given_equation_satisfy_forumla_lhs_structure(equation, formula_lhs):
        ...
        if node_type(formula_lhs.name) in {"u_", "p_"}: 
            if formula_lhs.name in variable_list.keys(): # check if that variable has previously appeared or not
                ...
                if node_type(formula_lhs.name) == "p_" and equation.name[:2] != "d_": # if formula has a p type variable, it only accepts integers
                    return False
...
def string_equation_helper(equation_tree):
    ...
    sign = {"f_add": "+", "f_mul": "*", "f_pow": "^", "f_poly": ",", "f_div": "/", "f_int": ",", "f_sub": "-", "f_dif": "?", "f_sin": "?", "f_cos": "?", "f_tan": "?", "f_eq": "=", "f_sqt": "?"} # operation symbols
...
def reduce(eq):
    ...
    return eq
import math_parser_2

while True:
    tmp = input(">>> ")
    if tmp.split(" ")[0] == "equation":
        eq = math_parser_2.take_input(tmp.split(" ")[1])
    elif tmp == "reduce":
        eq = reduce(eq)
        ...
        tmp2 = sorted([y for y in search(eq, 1000, [None, "formula-list-6/integrate_simple.txt", None]) if "f_int" not in y], key=lambda x: len(string_equation(x)))
        if tmp2 != []:
            eq = tmp2[0]
            print(string_equation(eq))
        else:
